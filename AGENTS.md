...
Developer engine for developer-facing tooling, extension management, registry workflows, and developer console features.

## Allowed Work

- Bug fixes and small enhancements for developer tools, API key screens, webhooks, logs, sockets, extensions, and registry workflows.
- Tests, documentation, and UI consistency improvements that follow existing Ember patterns.
- Developer workflow updates when docs and API reference impact are evaluated.

## Restricted Work

- Secret, token, API key, webhook signing, or registry publishing changes without explicit review.
- Large developer console workflow rewrites without approval.
- API or developer workflow changes without checking `fleetbase/postman` and `fleetbase/fleetbase.io`.
